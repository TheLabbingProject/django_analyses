...
class TupleInput(Input):
    value = JSONField()
    definition = models.ForeignKey(
        "django_analyses.TupleInputDefinition",
        on_delete=models.PROTECT,
        related_name="input_set",
    )

    @classmethod
    def validate_element_types(cls, value: tuple, expected_type: type) -> bool:
    ...
    def raise_not_tuple_error(self) -> None:
        raise ValidationError("TupleInput value must be a tuple instance!")

    def raise_incorrect_type_error(self) -> None:
        raise ValidationError(
            f"Tuple elements must be of type {self.expected_type}!"
        )
