...
:class:`ExponentCalculator` from the :doc:`analysis_integration` tutorial to create
a pipeline which which computes :math:`3^{x^2}` (where :math:`x` is the provided input).
