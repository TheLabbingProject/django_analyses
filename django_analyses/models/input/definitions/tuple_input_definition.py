...
from django_analyses.models.input.types.tuple_input import TupleInput
...
class TupleInputDefinition(InputDefinition):
    ...
    input_class = TupleInput

    def raise_not_tuple_error(self) -> None:
        raise ValidationError(
            _("Default TupleInputDefinition value must be a tuple instance!")
        )

    def raise_wrong_type_error(self) -> None:
        required_type = ListElementTypes[self.element_type].value.lower()
        raise ValidationError(
            _(f"Tuple elements must be of type {required_type}")
        )
